...
for (var i = 3;i < process.argv.length; i++) {
  if(process.argv[i] === 'shuffle') transform.shuffle(bitmap.pixelBuffer);
  ...
  if(process.argv[i] === 'order') transform.order(bitmap.pixelBuffer);
  if(process.argv[i] === 'randomize') transform.randomize(bitmap.colorPalette);
  if(process.argv[i] === 'greyScale') transform.greyScale(bitmap.colorPalette);
}
